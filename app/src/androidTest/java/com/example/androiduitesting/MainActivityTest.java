...
public class MainActivityTest {
    ...
    public void testActivitySwitch() {
        ...
        intended(hasComponent(ShowActivity.class.getName())); // Check that ShowActivity is launched
        Intents.release();
    }
    ...
    public void testCityNameConsistency() {
        ...
        // Click on city name on ShowActivity
        onData(anything()).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());
    ...
    public void testBackButtonReturnsToMainActivity() {
        ...
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Edmonton"));
        onView(withId(R.id.button_confirm)).perform(click());
        // Click on "Edmonton" to open ShowActivity
        onData(anything()).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());

        // Click the back button in ShowActivity
        onView(withId(R.id.backButton)).perform(click());

        // Verify city list is visible (means it's on MainActivity)
